...
TEST_F(CostmapToPolygonsDBSMCCHTest, dbScan)
{
  ...
  ASSERT_EQ(static_cast<size_t>(3), clusters.size());
  ASSERT_EQ(static_cast<size_t>(2), clusters[0].size()); // noisy points not belonging to a cluster
  ASSERT_EQ(static_cast<size_t>(costmap_to_polygons.parameters().max_pts_), clusters[1].size()); // first cluster at (0,0)
  ASSERT_EQ(static_cast<size_t>(costmap_to_polygons.parameters().max_pts_/2 + 1), clusters[2].size()); // second cluster at (1,1)
}
...
TEST(CostmapToPolygonsDBSMCCH, EmptyMap)
{
  ...
  std::vector< std::vector<costmap_converter::CostmapToPolygonsDBSMCCH::KeyPoint> > clusters;
  costmap_to_polygons.dbScan(clusters);
  ASSERT_EQ(static_cast<size_t>(1), clusters.size());    // noise cluster exists
  ASSERT_EQ(static_cast<size_t>(0), clusters[0].size()); // noise clsuter is empty
}
...
TEST(CostmapToPolygonsDBSMCCH, SimplifyPolygon)
{
  ...
  ASSERT_EQ(static_cast<size_t>(2), polygon.points.size());
  ...
  costmap_to_polygons.simplifyPolygon(polygon);
  ASSERT_EQ(static_cast<size_t>(3), polygon.points.size());
  for (size_t i = 0; i < polygon.points.size(); ++i)
  {
    ASSERT_FLOAT_EQ(original_polygon.points[i].x, polygon.points[i].x);
    ASSERT_FLOAT_EQ(original_polygon.points[i].y, polygon.points[i].y);  
...
TEST(CostmapToPolygonsDBSMCCH, SimplifyPolygonPerfectLines)
{
  ...
  costmap_to_polygons.simplifyPolygon(polygon);
  ASSERT_EQ(static_cast<size_t>(6), polygon.points.size());
